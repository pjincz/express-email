  ...
  obj.preview = function(locals) {
    return function(req, res, next) {
      ...
      if (req.query.cid) {
        ...
        return;
      } else if (!req.query.html && !req.query.text) {
        res.sendFile(path.join(__dirname, 'preview.html'));
        return;
      }

      // force re-render
      var preview_engine = new EmailTemplate(emailPath);
      var tmp = extend({}, res.locals, locals);
      preview_engine.render(tmp, function(err, result) {
        if (err) {
          next(err);
          return;
        }
